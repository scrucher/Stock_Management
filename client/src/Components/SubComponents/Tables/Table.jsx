...
// Main Component
const Table = forwardRef(({ path }, ref) => {
  ...
  const fetchData = async (path) => {
    try {
      ...
      setListAll(list_allData.data); // Assuming GetCategories returns an object with a 'list' property
    } catch (error) {
              ...
              <tbody>
                {list_all.map((data, index) => (
                  <tr key={index} className="border-t text-left">
                    <td className="p-2 w-1/4">{data.id}</td>
                    <td className="p-2 w-1/4">{data.name}</td>
